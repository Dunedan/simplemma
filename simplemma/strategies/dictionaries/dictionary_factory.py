...
SUPPORTED_LANGUAGES = [
    path.stem for path in DATA_FOLDER.iterdir()
    if path.is_file() and path.name.endswith(".plzma")]


def _load_dictionary_from_disk(langcode: str) -> BytesTrie:
    """
    ...
    with lzma.open(filepath, "rb") as filehandle:
        ...
        assert isinstance(pickled_dict, BytesTrie)
        return pickled_dict
